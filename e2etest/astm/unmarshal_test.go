...
type ComponentedTestMessage struct {
	Componented ComponentedRecord `astm:"TAG=C"`
}
...
type NoSequenceMessage struct {
	NoSeq NoSequenceRecord `astm:"TAG=N"`
}
...
type FullSingleASTMMessage struct {
	Header       lis02a2.Header       `astm:"TAG=H"`
	Manufacturer lis02a2.Manufacturer `astm:"TAG=M;ATR=optional"`
	Patient      lis02a2.Patient      `astm:"TAG=P"`
	Order        lis02a2.Order        `astm:"TAG=O"`
...
type MessagePORC struct {
	...
	OrderResults []struct {
		...
		CommentedResult []struct {
			Result  lis02a2.Result    `astm:"TAG=R"`
			Comment []lis02a2.Comment `astm:"TAG=C;ATR=optional"`
		} `astm:"GROUP"`
	} `astm:"GROUP"`
	Terminator lis02a2.Terminator `astm:"TAG=L"`
}
...
type CompleteOutOfStandardCustomRecord struct {
	F2           string  `astm:"POS=3"`
	F3           string  `astm:"POS=4"`
	Float32Value float32 `astm:"POS=5"`
	Float64Value float64 `astm:"POS=6"`
}
type MessageWithOutOfStandardCustomRecord struct {
	Header       lis02a2.Header                    `astm:"TAG=H"`
	CustomRecord CompleteOutOfStandardCustomRecord `astm:"TAG=X"`
	Terminator   lis02a2.Terminator                `astm:"TAG=L"`
}
...
type UnmarshalEnumRecord struct {
	Value UnmarshalEnum `astm:"POS=3"`
}
type UnmarshalEnumMessage struct {
	Record UnmarshalEnumRecord `astm:"TAG=E"`
}
...
type ReagentSubstructure struct {
	DescriptionOfReagent    string    `astm:"POS=1"  db:"description_of_reagent"`
	BarcodeOfReagent        string    `astm:"POS=2" db:"barcode_of_reagent"`
	LotNumberOfReagent      string    `astm:"POS=3" db:"lot_number_of_reagent"`
	ExpirationDateOfReagent time.Time `astm:"POS=4" db:"expiration_date_of_reagent"`
}
type AccessTimeComment struct {
	// 3.2.8 - ih_com_host_connection_manual_astm_1394_en_h009164_v1_8.pdf
	Reagents                    []ReagentSubstructure `astm:"POS=3"`
	TypeOfTestMedia             string                `astm:"POS=4.1" db:"type_of_test_media"`
	PlateOrIDCardBarcode        string                `astm:"POS=4.2" db:"plate_or_id_card_barcode"`
	LotNumberOfCassetteOrPlate  string                `astm:"POS=4.3" db:"lot_number_of_cassette_or_plate"`
	ExpDateForIDCardOrPlate     time.Time             `astm:"POS=4.4" db:"exp_date_for_id_card_or_plate"`
	IDCardOrPlateRealWellNumber int                   `astm:"POS=4.5" db:"id_card_or_plate_real_well_number"`
	Comment                     string                `astm:"POS=5" db:"comment"`
	FileName                    string                `astm:"POS=6" db:"file_name"`
}
type MessageTimeAccess struct {
	Header     lis02a2.Header     `astm:"TAG=H"`
	Comment    AccessTimeComment  `astm:"TAG=C"`
	Terminator lis02a2.Terminator `astm:"TAG=L"`
}
...
func TestGermanLanguage_Windows1252(t *testing.T) {
	...
	encodedMessageBytes := helperEncode(charmap.Windows1252, []byte(messageString))
	// Act
	err := parser.Unmarshal(encodedMessageBytes, &message, config)
	// Assert
	assert.Nil(t, err)
...
func TestGermanLanguage_ISO8859_1(t *testing.T) {
	...
	encodedMessageBytes := helperEncode(charmap.ISO8859_1, []byte(messageString))
	// Act
	err := parser.Unmarshal(encodedMessageBytes, &message, config)
	// Assert
	assert.Nil(t, err)
...
type TraceabilityReagentInfo struct {
	SerialNumber   string `astm:"POS=1"`
	UsedAtDateTime string `astm:"POS=2"`
	UseByDate      string `astm:"POS=3"`
}
type ManufacturerInfo struct {
	F2          string                    `astm:"POS=3"`
	Reagents    []string                  `astm:"POS=4"`
	ReagentInfo []TraceabilityReagentInfo `astm:"POS=5"`
}
type HoribaYumizenMessage struct {
	Manufacturer ManufacturerInfo `astm:"TAG=M;ATR=optional"`
	ExtraTests   struct {
		ArrayOfInt     []int     `astm:"POS=3"`
		ArrayOfFloat32 []float32 `astm:"POS=4"`
		ArrayOfFloat64 []float64 `astm:"POS=5"`
	} `astm:"TAG=E;ATR=optional"`
	Terminator lis02a2.Terminator `astm:"TAG=L"`
}
...
func TestUnmarshalSliceOfOneRecordType(t *testing.T) {
	...
	message := struct {
		ImageData []struct {
			...
			Field2          string `astm:"POS=4"`
			Field3          string `astm:"POS=5"`
			Stream1Encoding string `astm:"POS=6.1"`
			Stream1Data     string `astm:"POS=6.2"`
			Stream2Encoding string `astm:"POS=7.1"`
			Stream2Data     string `astm:"POS=7.2"`
		} `astm:"TAG=M"`
	}{}
	// Act
	err := parser.Unmarshal([]byte(messageString), &message, config)
...
type YumizenResultMessage struct {
	Header       lis02a2.Header                     `astm:"TAG=H"`
	OrderResults []YumizenPatientOrderResultMessage `astm:"GROUP"`
	Terminator   lis02a2.Terminator                 `astm:"TAG=L"`
}
type YumizenPatientOrderResultMessage struct {
	Patient            lis02a2.Patient           `astm:"TAG=P"`
	Comment            []lis02a2.Comment         `astm:"TAG=C;ATR=optional"`
	CommentedOrder     lis02a2.Order             `astm:"TAG=O"`
	Histograms         []YumizenStream           `astm:"TAG=M;ATR=subname:HISTOGRAM"`
	Matrices           []YumizenStream           `astm:"TAG=M;ATR=subname:MATRIX"`
	TraceabilityRecord YumizenTraceabilityRecord `astm:"TAG=M;ATR=subname:REAGENT"`
	CommentedResults   []lis02a2.ResultGroup     `astm:"GROUP"`
}
type YumizenTraceabilityRecord struct {
	MessageType       string   `astm:"POS=3"`
	TraceabilityNames []string `astm:"POS=4"`
	Traceability      []struct {
		Name  string `astm:"POS=1"`
		Date1 string `astm:"POS=2"`
		Date2 string `astm:"POS=3"`
	} `astm:"POS=5"`
}
type YumizenStream struct {
	...
	Field2      string `astm:"POS=4"`
	Field3      string `astm:"POS=5"`
	Stream1Type string `astm:"POS=6.1"`
	Stream1     string `astm:"POS=6.2"`
	Stream2Type string `astm:"POS=7.1"`
	Stream2     string `astm:"POS=7.2"`
}
